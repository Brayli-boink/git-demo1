            ...
            if (u) {
                ...
                onValue(ref(db, `foundPets`), (snapshot) => {
                    ...
                    const userPets = Object.entries(data)
                        .filter(([id, pet]) => pet.foundPoster === u.uid)
                        .map(([id, pet]) => ({ id, ...pet }));
                    setPets(userPets);
                });
            } else {
...
function confirmDelete() {
    remove(ref(db, `foundPets/${selectedPetId}`))
        .then(() => {
        ...
        >
          <div className="flex flex-col gap-6 items-center">
            ...
            <div className="flex flex-col text-center">
              <p className="font-semibold">{pet.petName || "N/A"}</p>
...
export default ReportedFoundPetCard;