...
function ReportFoundPetForm() {
    ...
    const [picture, setPicture] = useState("");
        ...
        useEffect(() => {
            ...
            function uploadFoundPet() {
        if (!user) return; // ensure user is logged in
    
        let storeLostPet = {
            foundPoster: user.uid,
            status: "found",
            petName: petName,
            species: species,
            breed: breed,
        ...
        set(ref(db, `/foundPets/${petId}`), storeLostPet)
            .then(() => {
                navigate("/reportedFoundPets");
            })
            .catch((error) => {
                console.error("Error uploading found pet:", error);
            });
    }
        ...
        <div className="w-full sm:w-[95%] md:w-[85%] h-auto flex flex-col bg-[rgba(255,255,255,0.21)] rounded-3xl border border-white  shadow-md">
            <div className='flex gap-4'>
                <div className="flex flex-col w-2/4 h-full px-4 py-4 rounded-t-3xl rounded-b-3xl">
                    ...
                    <p className='text-lg text-[#583523]' required > Pet Name </p>
                    ...
                    <p className='text-lg text-[#583523]'>Color/Marking</p>
                    ...
                    <p className='text-lg text-[#583523]'>Date Found<span className="text-red-600"> *</span></p>
